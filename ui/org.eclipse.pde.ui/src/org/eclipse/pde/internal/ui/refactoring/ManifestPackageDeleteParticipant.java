...
public class ManifestPackageDeleteParticipant extends PDEDeleteParticipant {
	...
	protected void addChange(CompositeChange result, IProgressMonitor pm) throws CoreException {
		...
		if (file.exists()) {
			// For each deleted package, remove it from Export-Package header
			for (Object element : fElements.keySet()) {
				if (element instanceof IPackageFragment pkg) {
					String packageName = pkg.getElementName();
					Change change = BundleManifestChange.createEmptyPackageChange(file, packageName, pm);
					if (change != null) {
						result.add(change);
					}
				}
